...
def experience_Pneumoniamnist(config, path, param, data_train_attack, X, Y, C, dp_mechanism, dp_epsilon, dp_delta, rate):
    print("START MNIST")
    ...
    bloodmnist_data = np.load('./data/pneumoniamnist.npz')
    ...
    for num_model_sahdow in range(config.general.number_shadow_model):
        ...
        model_shadow = PneumoniaMnist_CNN().to(device)
    ...
    print(C.shape)
    test_size_ = len(Y)
    X_data = np.random.randn(test_size_, 2)
    Y_data = np.random.randint(0, 1, size = [1, test_size_])   # 范围[)
    C_data = np.random.randint(0, 2, size = [1, test_size_])
