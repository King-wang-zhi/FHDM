...
def experience_Pathmnist(config, path, param, data_train_attack, X, Y, C, dp_mechanism, dp_epsilon, dp_delta, rate):
    print("START MNIST")
    ...
    bloodmnist_data = np.load('./data/pathmnist.npz')
    ...
    for num_model_sahdow in range(config.general.number_shadow_model):
        ...
        model_shadow = PathMnist_CNN().to(device)
    ...
    print(C.shape)
    test_size_ = len(Y)
    X_data = np.random.randn(test_size_, 9)
    Y_data = np.random.randint(0, 1, size = [1, test_size_])   # 范围[)
    C_data = np.random.randint(0, 9, size = [1, test_size_])
